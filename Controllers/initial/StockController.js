  ...
  try {
    ...
    if (verify) {
      const stockQuery = `INSERT INTO stocks(stock_id, stock_name, stock_type, stock_quantity, 
            ...
            '${req.body.stock_price}','0','0','${dateForToday}')`;
      database.query(stockQuery, (err, results) => {
        if (err) {
          res.status(400).json({
            success: false,
...
stock.put("/update/:stock_id", (req, res) => {
  try {
    ...
    if (verify) {
      const stockQuery = `UPDATE stocks SET stock_name='${req.body.stock_name}',stock_type='${req.body.stock_type}',
        stock_quantity='${req.body.stock_quantity}', stock_available='${req.body.stock_available}', stock_price='${req.body.stock_price}' 
